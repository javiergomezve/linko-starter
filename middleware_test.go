...
func Test_requestLogger(t *testing.T) {
	...
	if rr.Code != expectedStatusCode {
		t.Errorf("The statusCode is wrong: Expected %d Got %d", expectedStatusCode, rr.Code)
	}

	if logBuffer.String() != expectedLogString {
		t.Errorf("The expected log fail: Expected: %s Got %s", expectedLogString, logBuffer.String())
	}
}
